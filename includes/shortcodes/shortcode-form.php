    ...
    function tailor_shortcode_form( $atts, $content = null, $tag ) {
	    ...
	    if ( class_exists( 'WPCF7_ContactForm' ) ) {
		    ...
		    if ( empty( $contact_form ) ) {
			    $content = sprintf( '<p class="tailor-notification tailor-notification--warning">%s</p>', __( 'Please select a contact form to display', 'tailor' ) );
		    }
			else {
		    ...
		    $content = sprintf( '<p class="tailor-notification tailor-notification--warning">%s</p>', __( 'Please enable the Contact Form 7 plugin', 'tailor' ) );
	    }
