...
if ( ! function_exists( 'tailor_control_presets' ) ) {
    ...
    function tailor_control_definitions($choices) {
        return array(
	...
	function tailor_control_presets( $element, $control_ids = array(), $control_arguments = array(), $priority = 10 ) {
	    ...
	    // TODO: Push this and control_definitions into a cached value so we only populate once
        $choices = wp_cache_get('tailor_control_choices');
        if (!$choices) {
            $choices = array(
                'top'                   =>  __( 'Top', 'tailor' ),
        ...
        $control_definitions = wp_cache_get('tailor_control_definitions');
        if (!$control_definitions) {
            $control_definitions = tailor_control_definitions($choices);
        }

		wp_cache_add('tailor_control_definitions', $control_definitions);
		wp_cache_add('tailor_control_choices', $choices);
