...
if ( $facebook_link = esc_url( get_the_author_meta( 'facebook', $author_id ) ) ) {
	$link_title = esc_attr( sprintf( __( 'View %s\'s Facebook profile', tailor()->textdomain() ), $display_name ) );
	$social_links .=    '<li>' .
...
if ( $twitter_link = esc_url( get_the_author_meta( 'twitter', $author_id ) ) ) {
	$link_title = esc_attr( sprintf( __( 'View %s\'s Twitter page', tailor()->textdomain() ), $display_name ) );
	$social_links .=    '<li>' .
...
if ( $google_plus_link = esc_url( get_the_author_meta( 'googleplus', $author_id ) ) ) {
	$link_title = esc_attr( sprintf( __( 'View %s\'s Google Plus profile', tailor()->textdomain() ), $display_name ) );
	$social_links .=    '<li>' .
